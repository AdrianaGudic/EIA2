/*
Aufgabe: A4, Assoziative Skipiste
Name: Adriana Gudic
Matrikel: 256217
Datum: 10.11.2017
...
window.addEventListener("load", init);
    ...
    let cloudY: number[] = [];
    let skier: SkierInfo[] = []; //Interfacearray
...
//Animationsfunktion
    ...
    for (let i: number = 0; i < cloudX.length; i++) { //Animation Wolke
     ...
     for (let i: number = 0; i < skier.length; i++) { //Animation Skifahrer
         moveAndDrawSkier( skier[i] );
            if (skier[i].x > 1000, skier[i].y > 600) {
                skier[i].x = -20;
                skier[i].y = 200 + Math.random() * 100;
                }
         }
...
//Funktion Wolken
    ...
    function moveAndDrawSkier(_skier: SkierInfo): void {
        _skier.x += _skier.dx;
        _skier.y += _skier.dy;
        crc2.beginPath();
        crc2.arc(_skier.x, _skier.y - 10, 10, 1, 2 * Math.PI);
        crc2.fillStyle = "#000000";
        crc2.fill();
        crc2.fillStyle = _skier.color;
        crc2.fillRect(_skier.x - 4, _skier.y - 5, 5, 35);
        crc2.beginPath();
        crc2.moveTo(_skier.x - 20, _skier.y + 20);
        crc2.lineWidth = 2;
        crc2.lineTo(_skier.x + 35, _skier.y + 40);
        crc2.strokeStyle = "black";
        crc2.stroke();
        crc2.beginPath();
        crc2.moveTo(_skier.x - 20, _skier.y + 10);
        crc2.lineWidth = 3;
        crc2.lineTo(_skier.x + 35, _skier.y + 30);
        crc2.stroke();
